...
export function LandingNav() {
  ...
  // Lock body scroll while the mobile menu is open (locking the root element
  // keeps mobile browser chrome / fixed-header positioning stable — locking
  // <body> alone can make the navbar visually jump).
  useEffect(() => {
    if (!open) return
  ...
  return (
    ...
    {/* Mobile slide-in menu — rendered OUTSIDE the <header> on purpose:
        the header's backdrop-blur creates a containing block for fixed
        descendants, which would squish a fullscreen fixed overlay into the
        65px header box (transparent menu + floating buttons bug).
        Fully opaque bg + z-[60] keeps exactly one header row visible. */}
    <AnimatePresence>
      {open && (
        <motion.div
          initial={{ opacity: 0 }}
          ...
          className="fixed inset-0 z-[60] flex flex-col bg-fora-bg md:hidden"
          role="dialog"
          aria-modal="true"
        ...
        >
          ...
          <div
            className="space-y-3 border-t border-fora-border p-6 pb-[max(1.5rem,env(safe-area-inset-bottom))]"
          >
            <button
              type="button"
              onClick={() => navigate(ROUTES.REGISTER)}
